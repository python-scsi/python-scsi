...
_enums = ((key, rdi_enums.__dict__[key]) for key in rdi_enums.__dict__.keys()
...
class ReadDiscInformation(SCSICommand):
    ...
    _sdi_bits = {'disc_information_length': [0xffff, 0],
    ...
    _tri_bits = {'disc_information_length': [0xffff, 0],
                 'disc_information_data_type': [0xe0, 2],
    ...
    _pow_bits = {'disc_information_length': [0xffff, 0],
                 'disc_information_data_type': [0xe0, 2],
    ...
    def unmarshall_datain(cls,
        ...
        if data[2] >> 5 == cls.DISC_INFORMATION_DATA_TYPE.STANDARD_DISC_INFORMATION:
            ...
            result['number_of_sessions'] = result['number_of_sessions_msb'] * 256 + result['number_of_sessions_lsb']
            del result['number_of_sessions_msb']
            del result['number_of_sessions_lsb']
            result['first_track_number_in_last_session'] = result['first_track_number_in_last_session_msb'] * 256 + result['first_track_number_in_last_session_lsb']
            del result['first_track_number_in_last_session_msb']
            del result['first_track_number_in_last_session_lsb']
            result['last_track_number_in_last_session'] = result['last_track_number_in_last_session_msb'] * 256 + result['last_track_number_in_last_session_lsb']
        ...
        if data[2] >> 5 == cls.DISC_INFORMATION_DATA_TYPE.TRACK_RESOURCES_INFORMATION:
            decode_bits(data,
                        ...
                        result)
            return result
        if data[2] >> 5 == cls.DISC_INFORMATION_DATA_TYPE.POW_RESOURCES_DISC_INFORMATION:
            decode_bits(data,
                        cls._pow_bits,
                        result)
            return result

        raise NotImplementedError('Unknown disc information data type %d' % (data[2] >> 5))
