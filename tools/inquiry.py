...
def inquiry_standard(s):
    ...
    print("================")
    print(
        "PQual=%d  Device_type=%d  RMB=%d  version=0x%02x  %s"
        % (
    ...
    print(
        "NormACA=%d  HiSUP=%d  Resp_data_format=%d"
        % (i["normaca"], i["hisup"], i["response_data_format"])
    )
    print(
        "SCCS=%d  ACC=%d  TPGS=%d  3PC=%d  Protect=%d"
        % (i["sccs"], i["acc"], i["tpgs"], i["3pc"], i["protect"])
    )
    print("EncServ=%d  MultiP=%d  Addr16=%d" % (i["encserv"], i["multip"], i["addr16"]))
    print("WBus16=%d  Sync=%d  CmdQue=%d" % (i["wbus16"], i["sync"], i["cmdque"]))
    print("Clocking=%d  QAS=%d  IUS=%d" % (i["clocking"], i["qas"], i["ius"]))
    print(
        "  length=%d  Peripheral device type: %s"
        % (i["additional_length"] + 5, cmd.DEVICE_TYPE[i["peripheral_device_type"]])
    )
    print(
        "Vendor identification:",
        i["t10_vendor_identification"][:32].decode(encoding="utf-8", errors="strict"),
    )
    print(
        "Product identification:",
        i["product_identification"][:32].decode(encoding="utf-8", errors="strict"),
    )
    print(
        "Product revision level:",
        i["product_revision_level"].decode(encoding="utf-8", errors="strict"),
    )
...
def inquiry_supported_vpd_pages(s):
    ...
    print("===================================")
    print(
        "PQual=%d  Peripheral device type: %s"
        % (i["peripheral_qualifier"], cmd.DEVICE_TYPE[i["peripheral_device_type"]])
    )
...
def inquiry_logical_block_prov(s):
    ...
    print(
        "  LBPU=%d  LBPWS=%d  LBPWS10=%d  LBPRZ=%d  ANC_SUP=%d  DP=%d"
        % (i["lbpu"], i["lpbws"], i["lbpws10"], i["lbprz"], i["anc_sup"], i["dp"])
    )
    print(
        "  Provisioning Type=%d  [%s]"
        % (i["provisioning_type"], cmd.PROVISIONING_TYPE[i["provisioning_type"]])
    )
...
def inquiry_device_identification(s):
    ...
    for idx in range(len(_d)):
        print(
            "  Designation descriptor, descriptor length: %d"
            % (_d[idx]["designator_length"] + 4)
        )
        print(
            "    designator type:%d [%s]  code set:%d [%s]"
            % (
        ...
        print(
            "    association:%d [%s]"
            % (_d[idx]["association"], cmd.ASSOCIATION[_d[idx]["association"]])
        )
...
def inquiry_ata_information(s):
    ...
    print(
        ...
        i["sat_vendor_identification"].decode(encoding="utf-8", errors="strict"),
    )
    print(
        "SAT Product Identification:",
        i["sat_product_identification"].decode(encoding="utf-8", errors="strict"),
    )
    print(
        "SAT Product Revisions Level:",
        i["sat_product_rev_lvl"].decode(encoding="utf-8", errors="strict"),
    )
    ...
    if "identify" in i.keys():
        ...
        print(
            "       ",
            (
                specific_config[ident["specific_config"]]
                if ident["specific_config"] in specific_config.keys()
                else "reserved"
            ),
        )
        print(
            "    Device Serial number: %s"
            % ident["serial_number"].decode(encoding="utf-8", errors="replace")
        )
        print(
            "    Firmware Revision:",
            ident["firmware_rev"].decode(encoding="utf-8", errors="replace"),
        )
        print(
            "    Model Number:",
            ident["model_number"].decode(encoding="utf-8", errors="replace"),
        )
